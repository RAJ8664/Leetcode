class Solution {
    ...
    public int lenLongestFibSubseq(int[] arr) {
        ...
        for (int i = 0; i < n; i++) {
            if (map.containsKey(arr[i])) {
                ...
                temp = map.get(arr[i]); temp.add(i);
                map.put(arr[i], new TreeSet<>(temp));
            }
            else {
                map.put(arr[i], new TreeSet<Integer>());
                TreeSet<Integer> temp = new TreeSet<>(); temp.add(i);
                map.put(arr[i], new TreeSet<>(temp)); 
            }
        }
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                ...
                while (true) {
                    //check() -> finds the first index greater than prev_ind which is equal to a + b;
    ...
    private int check(int target, int low, int high) {
        if (map.containsKey(target)) {
            ...
            Integer current_higher = current.higher(low);
            if (current_higher != null) return  current_higher;
            return -1;
        }
        return -1;
    }
